document.addEventListener("DOMContentLoaded", function(){
	...
	on = function(el, evt, fn, opts = {}){
		...
		delegatorFn = e => {
			if( is.array(opts.target) ){
				...
				let target = e.target.closest(opts.target);
				if (!target) target = e.target;
				if(!el.contains(target)) return;
	...
	// Save | Download image
  	downloadImage = function(data, filename = 'untitled.jpeg') {
	...
	on(document, "click",function(e){
		if(is.element($mainImage) && (e.target.id === "filter-zoom-in" || e.target.id === "filter-zoom-out" || e.target.id === "filter-zoom-reset" || e.target.id === "resetAll" || e.target.id === "saveImage") ){
			...
			if(e.target.id === "saveImage"){
				try{
					var fileName = 'my-canvas.jpeg', imgType = "image/jpeg", imgQuality = 1.0, canvas = document.createElement("canvas"), 
